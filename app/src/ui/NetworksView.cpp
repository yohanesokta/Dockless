...
void NetworksView::Render(Renderer& renderer, float startX, float windowWidth, float windowHeight, 
    ...
    // 1. Heading
    renderer.DrawTextW(L"Networks", startX + padding, padding, contentWidth - padding * 2, 40.0f, 32.0f, D2D1::ColorF(1.0f, 1.0f, 1.0f), DWRITE_FONT_WEIGHT_SEMI_BOLD);
    ...
    if (createQuery.empty() && !createFocused) {
        renderer.DrawTextW(L"Create network, e.g. my-bridge-net", createInputLeft + 12.0f, createY + 6.0f, createInputWidth - 24.0f, 20.0f, 14.0f, textSecondary);
    } else {
    ...
    if (isCreateBtnHovered && mouseClicked && onAction && !createQuery.empty()) {
        std::string netName = WideToUtf8(createQuery);
        onAction(netName, "create");
    }
    ...
    if (searchQuery.empty() && !searchFocused) {
        renderer.DrawTextW(L"Search networks...", searchBoxLeft + 15.0f, searchY + 6.0f, searchBoxWidth - 30.0f, 20.0f, 14.0f, textSecondary);
    } else {
    ...
    // Filter Networks
    std::vector<DockerNetwork> filteredNetworks;
    if (searchQuery.empty()) {
        ...
        for (const auto& net : networks) {
            if (toLower(net.name).find(lowerSearch) != std::string::npos ||
                toLower(net.driver).find(lowerSearch) != std::string::npos ||
                toLower(net.id).find(lowerSearch) != std::string::npos) {
                filteredNetworks.push_back(net);
            }
        }
    }
    ...
    float colScopeX      = startX + padding + 400.0f;
    float colIdX         = startX + padding + 520.0f;
    ...
    renderer.DrawTextW(L"Scope", colScopeX, headerY, 100.0f, 20.0f, 14.0f, D2D1::ColorF(0.8f, 0.8f, 0.8f), DWRITE_FONT_WEIGHT_SEMI_BOLD);
    renderer.DrawTextW(L"Network ID", colIdX, headerY, 120.0f, 20.0f, 14.0f, D2D1::ColorF(0.8f, 0.8f, 0.8f), DWRITE_FONT_WEIGHT_SEMI_BOLD);
    ...
    float totalRowsHeight = filteredNetworks.size() * 50.0f;
    ...
    if (filteredNetworks.empty()) {
        renderer.DrawTextW(L"No networks to display.", startX + padding + 20.0f, rowY + 15.0f, contentWidth - padding * 2, 30.0f, 14.0f, textSecondary);
    } else {
        for (const auto& net : filteredNetworks) {
            if (rowY + 50.0f < clipTop || rowY > clipBottom) {
                rowY += 50.0f;
                continue;
            }
            
            std::wstring name(net.name.begin(), net.name.end());
            std::wstring driver(net.driver.begin(), net.driver.end());
            std::wstring scope(net.scope.begin(), net.scope.end());
            std::wstring shortId(net.id.begin(), net.id.end());
            
            float textBaseline = rowY + 15.0f;
            float iconY = rowY + 15.0f;
            float actionIconY = rowY + 13.0f;
            
            // Network icon (glyph 0xE17B - Network)
            renderer.DrawIcon((wchar_t)0xE17B, colIconX, iconY, 18.0f, textSecondary);
            ...
            renderer.DrawTextW(scope, colScopeX, textBaseline, 100.0f, 20.0f, 12.0f, textSecondary);
            renderer.DrawTextW(shortId, colIdX, textBaseline, 120.0f, 20.0f, 12.0f, textSecondary);
            
            // Delete action (Do not allow deleting default docker networks like bridge, host, none)
            bool isDefaultNetwork = (net.name == "bridge" || net.name == "host" || net.name == "none");
            ...
            if (!isDefaultNetwork) {
                ...
                // Draw a locked lock or disabled trash icon to indicate default network
                renderer.DrawIcon((wchar_t)0xE72E, colActionX, actionIconY, 18.0f, D2D1::ColorF(1.0f, 1.0f, 1.0f, 0.15f)); // Lock glyph
            }
